...
이름은 달의 여신 Selene 에서 왔고, 디자인은 거기서 나온다 — **미니멀하고 깨끗하게**,
밤하늘처럼 어두운 배경 위에 달빛 같은 실버.

- **색상(hue) 278 하나로 고정**한다. 푸른빛이 도는 보라다. 배경부터 전경까지
  명도와 채도만 움직여서, 두 팔레트가 우연히 붙어 있는 게 아니라 한 가족으로 읽히게 한다
- 강조색은 `--sl-glow` `oklch(80% 0.069 278)`. 램프 중 채도가 가장 높지만 절대값으로는
  낮아서 **보랏빛이 도는 실버**로 읽힌다. 순색 보라가 아니다
- 색은 `oklch()` 로 적는다. 명도가 지각적이라 `68% → 76% → 84%` 가 눈에 균일한 간격이다.
  hex 로는 같은 폭이 균일하지 않다. 픽커용으로 hex 를 주석에 병기한다
- 본문 **Inter** (사용자 계정에 정적 웨이트 7종 설치됨), 아이콘 **JetBrainsMono NFP**
  (Nerd Font 글리프는 Inter 에 없으므로 아이콘 규칙에 폰트를 따로 지정해야 한다)

색 값은 전부 `bar/styles/tokens.css` 에 있고, **그 아래로는 raw 색을 쓰지 않는다.**
팔레트를 또 바꿔도 그 파일 하나만 만지면 되게 하려는 것이다.

Catppuccin Mocha 로 시작했다가 위 방향으로 갈아엎었다. Mocha 값을 근거로 새 색을
제안하지 않는다.
